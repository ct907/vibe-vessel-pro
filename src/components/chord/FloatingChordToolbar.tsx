...
export function FloatingChordToolbar({
  ...
  useEffect(() => {
    ...
    return () => setToolbarExpanded(false);
  }, [effectiveExpanded, setToolbarExpanded]);
  ...
  const octaveValue = activeChord?.octave ?? 3;
  const octaveDisabled = !hasContext;
  const selectAllDisabled = !activeChord;
    ...
    <div className="fixed bottom-4 right-4 z-40 pointer-events-none">
      <button
        ...
        className={cn(
          ...
          "h-14 w-14 rounded-full flex items-center justify-center btn-sculpt-amber",
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={cn(
              ...
              hasSelection || activeChord ? "font-mono-chord text-foreground" : "text-muted-foreground",
            )}
          >
            {middleLabel}
          </span>
          ...
          <Button
            ...
            className={cn(
              "h-9 w-9",
              isActiveInMultiSet && "bg-[var(--ink-soft)] text-[var(--paper-card)] hover:bg-[var(--ink-soft)] hover:text-[var(--paper-card)]",
            )}
            disabled={!activeChord}
            onClick={onToggleMultiSelectActive}
            aria-label="Toggle multi-select for active chord"
            title="Add to / remove from multi-select"
          >
